...
function decorateEditor(editor: EditorComponent): EditorComponent {
  ...
  editor.render = (width: number): string[] => render(width).map(highlightDynamaxTokens);
  return editor;
}
  ...
  const installEditor = (ctx: ExtensionContext): void => {
    ...
    const installation: DynamaxEditorInstallation = {
      ...
      factory: (tui, theme, keybindings) => {
        if (!previousFactory) {
          return decorateEditor(new CustomEditor(tui, theme, keybindings));
        }

        try {
          return decorateEditor(previousFactory(tui, theme, keybindings));
        } catch (error) {
          ctx.ui.notify(
